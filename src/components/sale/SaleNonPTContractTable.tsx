...
const SaleNonPTContractTable = ({ searchText }: { searchText: string }): ReactElement => {
  ...
  useEffect(() => {
    ...
    const fetchNonPtServices = async () => {
      try {
        const response = await axiosPrivate.get('/nodejs/non-ptcontract/getNonPtServices');
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await axiosPrivate.put(
        `/nodejs/non-ptcontract/updateNonPTContract/${editingNonPTContract.id}/`,
        {
  ...
  const handleSaveAdd = async () => {
    ...
    try {
      const response = await axiosPrivate.post(
        `/nodejs/non-ptcontract/addNonPTContract/`,
        {
          startDate: editingNonPTContract.start_date,
  ...
  const handleDelete = async (id: string) => {
    ...
    if (
      ...
      try {
        const response = await axiosPrivate.post('/nodejs/non-ptcontract/deleteNonPTContractMultiple', {
          ids: idsToDelete,
        });
